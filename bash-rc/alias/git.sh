...
function gccd() {
  ...
  # Extract repo URL from arguments
  for arg in "$@"; do
    if [[ "$arg" =~ ^(ssh://|git://|ftps?://|https?://|.*@) ]]; then
      repo="$arg"
      break
    fi
  done

  # If repo wasn't matched above, assume it's the last argument
  if [ -z "$repo" ]; then
    for repo in "$@"; do :; done
  fi
...
if version_gte "$git_version" "2.8"; then
...
alias gpristine='DEFAULT_BRANCH=$(git symbolic-ref refs/remotes/origin/HEAD 2>/dev/null | sed "s@^refs/remotes/origin/@@" || git remote show origin 2>/dev/null | grep "HEAD branch" | cut -d" " -f5 || echo "main") && echo "Resetting to $DEFAULT_BRANCH" && git checkout $DEFAULT_BRANCH && git reset --hard && git clean --force -dfx'
...
if version_gte "$git_version" "2.13"; then
...
#)
#
#for old_alias in "${!deprecated_aliases[@]}"; do
#  new_alias="${deprecated_aliases[$old_alias]}"
#  eval "function $old_alias() {
#    echo -e \"\033[33m[oh-my-zsh] '\033[31m$old_alias\033[33m' is a deprecated alias, using '\033[32m$new_alias\033[33m' instead.\033[0m\" >&2
#    $new_alias \"\$@\"
#  }"
#done
